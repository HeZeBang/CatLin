...
app.get("*", (req, res) => {
  res.sendFile(path.join(reactPath, "index.html"), (err) => {
    if (err) {
      console.log("Error sending dist/index.html:", err.status || 500);
      res.status(err.status || 500).send("Error sending dist/index.html - have you run `npm run build`?");
    }
  });
});
